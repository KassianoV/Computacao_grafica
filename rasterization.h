...
//Função intersecta
float intersecta(vec2 A, vec2 B, int ys ){

	float x =(((B[0]-A[0])*(ys-A[1]))/(B[1]-A[1])) + A[0]; 
	if((ys==B[1]) && (ys==A[1])){ //yb==ys==ya
		return A[0];

	}else if((B[1] > ys && A[1] > ys) || (B[1] < ys && A[1] < ys)){ // y acima ou abaixo da aresta
		return NAN;
...
std::vector<Pixel> scanline(const Tri& P){
	...
	int ymin = ceil(std::min({A[1], B[1], C[1]}));
	int ymax = floor(std::min({A[1], B[1], C[1]}));

	std:: vector<Pixel> out;
	Pixel p;
	for(p.y = ymin; p.y <= ymax; p.y++){
		float AxB = intersecta(A,B,p.y);
		float BxC = intersecta(B,C,p.y);
